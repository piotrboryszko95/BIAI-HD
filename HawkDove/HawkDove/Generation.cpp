...
Generation::Generation(int populationSize, int mutationFactor)
{
	...
	for (int i = 0; i < populationSize; i++) {
		birds[i] = new Bird(genRandomInt(), i, mutationFactor);
	}
...
{
	int replacementIndex = (int) (replacementFactor * populationSize / (double)100);
...
void Generation::crossOver(Bird &bird1, Bird &bird2)
{
	...
	unsigned int tmp2 = bird2.getGenotype();
	tmp1 <<= 4 * sizeof(int);
	tmp1 >>= 4 * sizeof(int);
	tmp2 >>= 4 * sizeof(int);
	tmp2 <<= 4 * sizeof(int);
